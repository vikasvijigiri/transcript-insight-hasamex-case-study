...
- **Rate limiting via `slowapi`**: `/qa`, `/themes`, and `/chat` — the three endpoints that already make paid, per-request Anthropic calls — have zero limiting today (this was miscounted as "two" earlier in this process; corrected here). OWASP API4 "Unrestricted Resource Consumption." Per-IP limits on these three specifically, not the whole app; no Redis needed at single-process scale. `/transcript` (§7a) doesn't call Anthropic at all — it serves already-parsed local file text — so it doesn't need cost-protection rate limiting for the same reason; a generic higher-ceiling limit is fine if applied for consistency, but it's not solving the cost problem the other three are.
...
- **Correlation/request ID + cache hit/miss logging**: server-generated request ID (never trust an inbound header), attached to every log line and returned in a response header; cache hit/miss logged at each `cache.read_cache`/`write_cache` call site (currently silent). Note: token counts, latency, and provider name are **already** logged per LLM call (`providers/anthropic_provider.py`, `providers/openai_compatible_provider.py`) — this was miscounted as a gap earlier in this process from only reading `main.py`, corrected in `design/OBSERVABILITY_CHECKLIST.md`. The one real refinement there: promote those already-logged fields from logfmt-in-message text to proper structured JSON fields (via the `extra_fields` mechanism `logging_config.py` already supports but the providers don't use), named per OpenTelemetry's GenAI Semantic Conventions so a future real OTel migration is a relabeling exercise, not a rewrite.
...
- **Eval harness scoring logic gets unit test coverage** (`backend/evals/`): corrected after actually reading `run_eval.py` — the citation-accuracy scoring this bullet originally proposed building already exists (`run_case()` already requires ≥1 verbatim-verified citation for golden cases and zero citations at all for trap cases). What's actually missing is that this scoring logic itself has zero test coverage — a regression in it would silently make the eval harness always pass, unnoticed, since the harness never runs in CI. Fix: unit-test `run_case()`'s scoring with a fake `AskResult` (same no-live-call pattern as the main pytest suite), not new scoring logic. Full correction in `design/TESTING_QA_CHECKLIST.md`.
...
- ~~`npm ci` in the frontend CI job~~ — checked `.github/workflows/ci.yml` directly: already in place, not a gap (corrected after miscounting it earlier). The backend gets the equivalent once CI switches to `uv sync --frozen` as part of the `uv` migration (§7c).
